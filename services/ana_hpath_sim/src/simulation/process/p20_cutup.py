...
def cutup_bms(self: Specimen) -> None:
    ...
    block = Block(f"{self.name()}.", env=env, parent=self, block_type="small surgical")
...
def cutup_pool(self: Specimen) -> None:
    ...
    block = Block(f"{self.name()}.", env=env, parent=self, block_type="large surgical")
    self.blocks.append(block)
    env.specimen_data[self.name()]["num_blocks"] = 1
    ...
    if self.prio == Priority.URGENT:
        self.enter_sorted(
            env.processes["cutup_pool_to_processing"].in_queue, Priority.URGENT
        )
    else:
...
def cutup_large(self: Specimen) -> None:
    ...
    for _ in range(n_blocks):
        block = Block(f"{self.name()}.", env=env, parent=self, block_type=block_type)
        self.blocks.append(block)
    ...
    if self.prio == Priority.URGENT:
        self.enter_sorted(
            env.processes["cutup_large_to_processing"].in_queue, Priority.URGENT
        )
    else:
