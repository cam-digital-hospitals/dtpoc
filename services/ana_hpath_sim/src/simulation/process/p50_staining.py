...
from .__core import Batch, BatchingProcess, CollationProcess, DeliveryProcess, Process
    ...
    for block in self.blocks:
        for slide in block.slides:
            if slide.data["slide_type"] == "megas":
                slide.enter_sorted(
                    env.processes["batcher.staining_megas"].in_queue, self.prio
                )
            else:
                slide.enter_sorted(
                    env.processes["batcher.staining_regular"].in_queue, self.prio
                )
...
def post_staining(self: Specimen) -> None:
    ...
    if self.prio == Priority.URGENT:
        self.enter_sorted(
            env.processes["staining_to_labelling"].in_queue, Priority.URGENT
        )
    else:
