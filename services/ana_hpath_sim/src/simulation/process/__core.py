...
from ...conf import ARR_RATE_INTERVAL_HOURS, RESOURCE_ALLOCATION_INTERVAL_HOURS
from ..specimens import Batch, Component, Priority, Specimen

if TYPE_CHECKING:
    from ..input_data import ArrivalSchedule, ResourceSchedule
...
class ArrivalGenerator(Component):
    ...
    def __init__(
        self, *args, schedule: "ArrivalSchedule", env: "Model", **kwargs
    ) -> None:
        """Constructor.
...
class Process(sim.Component):
    ...
    def __init__(
        self, *args, in_type: Type, fn: Callable, env: Environment = None, **kwargs
    ) -> None:
        """Constructor.
    ...
    def setup(  # pylint: disable=arguments-differ
        self, in_type: Type[Component], fn: Callable
    ) -> None:
...
class BatchingProcess(Component):
    ...
    def process(self) -> None:
        ...
        while True:
            batch_size = (
                self.batch_size() if callable(self.batch_size) else self.batch_size
            )
...
class CollationProcess(Component):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # super().__init__ consumes args and a bunch of kwargs and passes the rest to setup()
        super().__init__(
            *args, **kwargs, env=env, counter_name=counter_name, out_process=out_process
        )

    def setup(  # pylint: disable=arguments-differ
        self, counter_name: str, out_process: str
    ) -> None:
    ...
    def process(self) -> None:
        ...
        while True:
            ...
            data = (
                env.specimen_data[item.parent.name()]
                if isinstance(item.parent, Specimen)
                else item.parent.data
            )
            if len(self.dict[key]) == data[self.counter_name]:
                item.parent.enter_sorted(
                    env.processes[self.out_process].in_queue, item.parent
                )
                del self.dict[key]
...
class DeliveryProcess(Component):
    ...
    def process(self) -> None:
        ...
        while True:
            ...
            delivery_prio = (
                entity.prio if not isinstance(entity, Batch) else Priority.ROUTINE
            )
